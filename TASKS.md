...
- [ ] **P4d-5: Pooled, common-space service-usage embedding** — stack a `(geoid, year)` × feature usage-share matrix from `{geo_key}_srtype_metrics_{year}.parquet` across all available years (row-normalized to shares, `_MIN_GEO_SRTYPE_N`-suppressed, sparse features/geo-years dropped). **Feature-set toggle** between two matrices: *high-level categories* (~12-15 department-prefix shares — dense, every geo-year nonzero, the safer space to validate first) and *top-100 individual SRTypes by citywide volume* (richer/more specific signal, sparser per geo-year, more exposed to suppression). Fit `StandardScaler` + `sklearn.decomposition.PCA` to 2D **once on the full pooled matrix** (≈560 CSA-year or ≈1,990 tract-year rows) — not per year — so every point is projected through the same fixed transform and shares one stable space. Support both geographic grains via the shared `geo_level` toggle (CSA's denser cells make the more trustworthy first fit; tract adds resolution once validated). **Validation gate before building anything on top of this**: report % variance explained by the first two components for each (feature-set × geo-grain) combination — if too low to trust a 2D read, that's the signal to add components, reconsider the feature set, or fall back to a non-PCA method (e.g. UMAP) before investing in clustering.
- [ ] **P4d-5b: Animated year-by-year trajectory view** — an animated slider (Plotly `frames` / `animation_frame`, or a manual `st.slider` + redraw) steps through years in the common space, so a geography's movement reads as motion rather than a static smear of ~2,000 overlapping points. Color-by stays user-selectable (demographic continuous scale, or saturation of a selected feature's usage share) and updates per frame. Answers: *"are areas continuous or clustered in what they request, is that stable or shifting year to year, and do demographically similar areas move together or apart?"*
- [ ] **P4d-6: Demographic-space embedding — the inverse view (single-snapshot, ACS 2023)** — place the same geographies in 2D from their demographic profile (likely a direct scatter or light PCA, since the input is low-dimensional), colored by service-side variables: predominant SRType (categorical), `requests_per_1k`, or `median_days_to_close`, user-selectable. Answers the inverse question: *"do areas that look alike demographically also look alike in how they use and experience 311?"*
- [ ] **P4d-6a: Expand the demographic feature set** — `{tract,csa}_demographics.csv` today carries only `pct_black`, `pct_white`, `median_income`. Extend the ACS pull in `metrics.py`'s demographics stage to add: age-group shares, Hispanic/Latino ethnicity share (`pct_hispanic` — distinct from race in Census conventions; a tract can be majority-White *and* majority-Hispanic, so this isn't redundant with the existing race split), bachelor's-degree-or-higher attainment rate, and 1-2 more ACS variables (candidates: poverty rate, owner-occupied housing share, vehicle access — whichever rounds out a "who lives here and how" picture without overlapping what's already there). Year-independent, one-time pipeline change — same `--stage demographics` pattern, more columns; feeds both P4d-6's embedding and gives Tab 4/5's existing race/income split richer company.
...
- [ ] **P4d-9: Wire into `app.py` as the third tab + new dependency** — insert `"Area Embedding"` between `"Services"` and `"Equity"` in `st.tabs([...])`; add `scikit-learn>=1.3.0` to `app/requirements.txt` (`PCA` + `StandardScaler` + clustering, all in the same package — no extra dependency for P4d-7b). Note `umap-learn` as a future enhancement once the PCA structure is validated — its `numba`/LLVM dependency makes it a much heavier install.
...
**Tab 2 / Services** (P4d-1 → 4, pure operations — the simplest, and establishes shared category-table/pill scaffolding) → **Tab 5 / Service Equity** (P4d-10 → 13, directly extends Tab 2's table with the equity comparison pattern from `equity_distributions.py`) → **Tab 6** (P4d-14 → 17, stratified scores generalize Tab 5's single-category comparison to every category, then add the regression panel) → **Tab 3** (P4d-5 → 9, independent of the category tabs once Stage 0 data is in hand; build both embeddings, validate them, *then* cluster into peer groups at P4d-7b once the spaces are stable) → propagate `peer_group` back into Operations, Services, Service Equity, and Equity per the space-appropriate boundary noted at P4d-7b → wire the two remaining new tabs into `app.py` as one coordinated change to the tab list (in final order: Operations, Services, Area Embedding, Equity, Service Equity, Equity Adjusted for Service Mix), imports, and sidebar description.

> **Status (2026-06-06)**: Tabs 2 and 5 (✅ **Services** and **Service Equity**) have shipped — both verified via `AppTest`, merged to `main`. Remaining build order is **Tab 6** (P4d-14 → 17) → **Tab 3** (P4d-5 → 9) → re-order/insert both into `app.py`'s `st.tabs([...])` and the sidebar.
