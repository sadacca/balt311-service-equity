...
- [ ] **P4d-1: Citywide SRType performance table, trended across years** — all SRTypes with their core operational metrics (volume, closure rate, median days to close, on-time rate, % resident-initiated) for the selected year, sortable by any metric; category pills above filter by department prefix. Reuse `_CATEGORY_NAMES`, `_EXCLUDED_CATEGORIES`, `_extract_categories()`, and the pill pattern from `operations_panel.py`, and `_load_srtype_history()` for the multi-year frame (consider promoting these to a shared constants module given they're now used in two tabs).
- [ ] **P4d-2: Among-category comparison** — a chart (grouped bars, dot-plot, or small multiples) putting categories side by side on usage volume and time-to-close, so differences *among* categories are visible at a glance — e.g. "rodent control runs 3x longer than streetlight repair." No demographic split; this is purely "how do these services compare to each other operationally."
- [ ] **P4d-3: Within-category year-over-year + geographic breakdown** — selecting a category shows (a) its trend across all available years (volume and median-days-to-close bars, reusing the year-over-year detail pattern already built for `_srtype_charts` in `operations_panel.py`) and (b) a choropleth of its volume or speed across tracts/CSAs (`build_choropleth()` from `map_view.py`, `sequential=True`). Geographic variation is shown as-is — **not** filtered through a demographic lens; that comparison is reserved for Tab 5.
- [ ] **P4d-4: Wire into `app.py` as the second tab** — insert `"Service Category Explorer"` immediately after `"Operations"` in `st.tabs([...])`; pass through `DATA_DIR`, `geo_key`, `year`, `geojson`, `featureidkey`, `MAPBOX_TOKEN` (no `demographics` argument needed — keeps the "no equity content" boundary explicit in the function signature itself).
