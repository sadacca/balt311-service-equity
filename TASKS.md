...
- [ ] **P4d-7b: Cluster the embedding into named peer groups — the real payoff of building this view** — run a clustering algorithm (start with `sklearn.cluster.KMeans` or `AgglomerativeClustering`, k chosen via silhouette score or a user-facing slider) on the 2D embedding (or the pre-PCA feature matrix — try both, prefer whichever gives more stable, interpretable groups) in *both* spaces. Auto-generate a short descriptive label per cluster from its dominant features (e.g. "High solid-waste / majority-Black tracts" from top SRType shares + demographic skew, or "Low-volume / mixed-income" from low total requests + income spread near the citywide median) and let the user rename/accept labels. Persist the assignment as `geoid → peer_group` so it becomes a reusable categorical dimension — exposed as a filter/group-by option (alongside the existing top-SRType filter pattern in the Equity tab and the category-pill pattern in Operations) in **other tabs and charts**: Operations time series, Category Explorer ranking/equity comparison, and the Equity tab's distribution charts and trend. This turns the embedding from a one-off visualization into the analytical backbone for "compare areas like mine" across the whole app — directly fulfilling Phase 4b's peer-comparison goal (P4b-1, P4b-3 through P4b-5) without needing a separate nearest-neighbor system.
...
- [ ] **P4d-9: Wire into `app.py` + new dependency** — add `"Area Embedding"` tab; add `scikit-learn>=1.3.0` to `app/requirements.txt` (`PCA` + `StandardScaler` + clustering, all in the same package — no extra dependency for P4d-7b). Note `umap-learn` as a future enhancement once the PCA structure is validated — its `numba`/LLVM dependency makes it a much heavier install.
...
Data is already in place (Stage 0 ✅) → Tab 1 ranking → Tab 1 choropleth → Tab 1 within-type equity comparison → Tab 3 stratified scores (extends the Tab 1 comparison to every SRType) → Tab 3 regression → Tab 2 embeddings, *then* Tab 2 clustering/peer-group naming (P4d-7b — do this only once both embedding spaces are validated and stable, since the cluster labels are meant to be reused elsewhere) → propagate `peer_group` as a filter/group-by option back into Operations, Category Explorer, and Equity → wire all three tabs into `app.py` together as one coordinated change to the tab list, imports, and sidebar description.
...
- **Tab 2 — peer groups (P4d-7b)**: cluster count is stable across re-runs (or seeded for reproducibility); auto-generated labels plausibly describe each cluster's dominant features; `peer_group` filter/group-by, once wired into Operations / Category Explorer / Equity, narrows those views to the expected subset of geographies and updates their charts accordingly.
