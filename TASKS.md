...
- [x] **P4d-1: Citywide SRType performance table, trended across years** — all SRTypes with their core operational metrics (volume, closure rate, median days to close, on-time rate, % resident-initiated) for the selected year, sortable by any metric; category pills above filter by department prefix. *Done — `_table_and_selection()`. The shared scaffolding (`_CATEGORY_NAMES`, `_EXCLUDED_CATEGORIES`, `_extract_categories()`, `category_pills()`, `_load_srtype_history()`, `_load_geo_srtype_metrics()`, `_MIN_GEO_SRTYPE_N`) was promoted out of `operations_panel.py` into a new shared module `app/components/srtype_shared.py` (as suggested) — both `operations_panel.py` and `category_explorer.py` now import from there.*
- [x] **P4d-2: Among-category comparison** — a chart putting categories side by side on usage volume and time-to-close, so differences *among* categories are visible at a glance — e.g. "rodent control runs 3x longer than streetlight repair." No demographic split. *Done — `_among_category_fig()`: a bubble chart, one point per department category, x = total volume, y = volume-weighted median days to close, size = volume (`_category_aggregates()` does the prefix rollup).*
- [x] **P4d-3: Within-category year-over-year + geographic breakdown** — selecting a category shows (a) its trend across all available years (volume and median-days-to-close bars, reusing the year-over-year detail pattern already built for `_srtype_charts` in `operations_panel.py`) and (b) a choropleth of its volume or speed across tracts/CSAs (`build_choropleth()` from `map_view.py`, `sequential=True`). Geographic variation is shown as-is — **not** filtered through a demographic lens. *Done — `_within_type_trend()` + `_geo_breakdown()`; both render only once a table row is selected, with an explicit prompt shown otherwise.*
- [x] **P4d-4: Wire into `app.py` as the second tab** — insert `"Service Category Explorer"` immediately after `"Operations"` in `st.tabs([...])`; pass through `DATA_DIR`, `geo_key`, `year`, `geojson`, `featureidkey`, `MAPBOX_TOKEN` (no `demographics` argument needed — keeps the "no equity content" boundary explicit in the function signature itself). *Done — `render_category_explorer()` wired in at `app.py`; sidebar tab description added; verified via `streamlit.testing.v1.AppTest` (renders all three tabs, no exceptions, table/pills/bubble-chart/drill-down render correctly for 2025 data).*
