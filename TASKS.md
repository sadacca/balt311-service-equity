...
> **Status (2026-06-07)**: Tabs 2, 3, and 5 (✅ **Services**, **Area Embedding**, and **Service Equity**) have shipped — all verified via `AppTest`, merged to `main`. Tab 3 ✅ live. Remaining: **Tab 6** (P4d-14 → 17) — once complete, re-order/insert into `app.py`'s `st.tabs([...])` and the sidebar. P4d-7b (quadrant grouping ✅, full KMeans cluster labels + cross-tab `peer_group` propagation open) may optionally feed Tab 6's ranking panel once it lands.
...
- **Tab 3 / Area Embedding**: ✅ verified via `AppTest` — both views (`Service usage` ↔ `Demographic profile`, via the `View` radio) render with no exceptions. Combined tract+CSA scatter displays tracts (small dots) and CSAs (large labeled bubbles) in a single shared PCA space; ~10% of CSA names labeled via farthest-point sampling. Quadrant backgrounds (NW/NE/SW/SE, divided at median x/y) render correctly; 100% stacked category-mix bar, predominant-SRType distribution bar, and four-column CSA neighborhood list all render below the scatter. Usage view: animated scatter (one point per `(geoid, year)`), color fixed to median income, variance-explained caption non-trivial. Demographic view: one static point per geography (ACS 2023 snapshot, no animation), color fixed to predominant SRType for the selected year. No geo_level toggle in this tab — the Areas tab loads all data internally and does not read `st.session_state["geo_level"]`.
