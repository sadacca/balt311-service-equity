...
> **Status (2026-06-13)**: Tabs 2, 3, 5, and now **Tab 6** (✅ **Services**, **Area Embedding**, **Service Equity**, **Mix-Adjusted Equity**) have shipped — all verified via `AppTest`. Tab 6 is an **initial draft** on `claude/tab-6-initial-draft-iimghr`, implementing P4d-14 → P4d-17; the full six-tab arc (Operations → Services → Area Service Usage → Equity → Service Equity → Mix-Adjusted Equity) now renders end to end with zero exceptions. P4d-7b (quadrant grouping ✅, full KMeans cluster labels + cross-tab `peer_group` propagation open) may optionally feed Tab 6's ranking panel once it lands. Possible follow-ups before merge: a read-through of the whole arc for bridging-caption gaps, and a memory check on the tract-grain regression for Streamlit Cloud (CSA is the app-wide default, which sidesteps it).
...
> **Partially realized in-app (Tab 6, 2026-06-13)**: the per-geography mix-normalized metric is now computed **in-app** for the selected year/metric by `compute_normalized_geo_metrics()` in `equity_adjusted.py` (volume-weighted `actual` vs. `expected` per geography, `residual = actual − expected`), and surfaced as a zero-centered **residual choropleth** (`_residual_choropleth_fig`) and an **actual-vs-expected scatter** colored by share Black (`_actual_expected_scatter_fig`). P4e-1/P4e-2 below are still worth doing as a **precomputed pipeline stage** (faster load, available to every tab, and lets P4e-3→5 add adjusted columns to the Equity tab / distributions / trend) — but the headline per-geography view exists now without a pipeline run. *(Note: the in-app version uses a signed difference `actual − expected` centered at 0; P4e-1 as written uses a ratio centered at 1.0 — reconcile to one convention when the stage is built.)*
