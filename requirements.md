...
| 6 | **Mix-Adjusted Equity** *(✅ live — in-app label; planning name "Equity Adjusted for Service Mix")* | Are the equity gaps surfaced in tabs 4–5 explained by *which* services an area requests (mix-driven) or by *how* the same service is delivered (delivery-driven)? | (a) Year-over-year trend of the raw geo-level vs. volume-weighted within-type "mix-adjusted" overlap score (race + income, fixed `[0,1]` axis with threshold bands); (b) per-neighborhood **direct-standardization** view — every geography reweighted to the citywide service mix, as a residual choropleth + raw-vs-adjusted scatter (computed record-level in the pipeline `adjusted` stage, since a median does not decompose into a weighted mean of per-type medians); (c) within-type SRType equity ranking with distribution drill-down; (d) WLS fixed-effects regression panel (coefficient table + CI plot) as an independent check |
...
Stage 2 — App (reads only from data/processed/, no network dependencies at runtime)
  ...
  app/components/equity_adjusted.py    → Mix-Adjusted Equity tab — Tab 6 (✅ live; Phase 4d/4e — normalized trend + record-level direct-standardization map/scatter + within-type ranking + regression)
